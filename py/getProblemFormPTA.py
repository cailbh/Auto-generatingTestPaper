...
with open(inputDir, "r", encoding="utf-8") as f:
    ...
    for problem in content:
        ...
        print(cont)
        id = problem['id']
        url = "https://pintia.cn/api/problems/"+str(id)
        headers = result_json['request']['headers']
        time.sleep(0.5)

        ret = requests.get(url, headers=head)
        # cur_rel = ret.json()#['problems']
        final_result.append(ret.json()['problem'])
#problems:
#         ----------
# while i == 200:
#     url = "https://pintia.cn/api/problems/1701115978840051712"
